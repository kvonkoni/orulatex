...
TITLE Compile OrulaPath: %~n1%~x1 - lualatex
...
FIND /I "orulapath" %filename%.tex >nul
IF %errorlevel% EQU 1 (
	ECHO Error: Document does not appear to be a valid OrulaPath:
	ECHO Must contain "\documentclass[<options>]{orulapath}"
	PAUSE
	EXIT /b
)
